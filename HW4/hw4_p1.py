...
def matrix(ans):
	...
	for i in range(10):
		...
		print(" "*3+"+---"*9+"+")
		if i!=10:
			print("",i,"|",ans[i][1],"|",ans[i][2],"|",ans[i][3],"|",ans[i][4],"|",ans[i][5],"|",ans[i][6],"|",ans[i][7],"|",ans[i][8],"|",ans[i][9],"|")
...
while True:
	...
	#建立空變數
	ans=[[' ']*11,[' ']*11,[' ']*11,[' ']*11,[' ']*11,[' ']*11,[' ']*11,[' ']*11,[' ']*11,[' ']*11,[' ']*11]
	#設定各種用來判斷的變數
	message=0
	h=0
	mines=[[-1,-1]]
			...
			for l in range(8):
				for k in range(81):
					for i in range(1,10):
						for j in range(1,10):
							if ans[i][j]==' ':
								if base[i][j]==l and (ans[i+1][j]==0 or ans[i-1][j]==0 or ans[i][j+1]==0 or ans[i][j-1]==0 or ans[i-1][j-1]==0 or ans[i-1][j+1]==0 or ans[i+1][j-1]==0 or ans[i+1][j+1]==0):
									ans[i][j]=base[i][j]
